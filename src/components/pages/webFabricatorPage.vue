               ...
               <div class="site-content">
                  <div class="site-content__inner ">
                     <div class="site-content__inner_2 block_padding">
                       <webSiteFabricatorLeft
                       :list="category__list"
                       :closeList="category__list"
                       />
                       <div v-if="fabricator" class="site-content__right">
                           <main class="site-main">
                               <div class="site-main__inner">
                                   <h1>{{ this.fabricator.title }}</h1>
...
import webSiteFabricatorLeft from '../models/site__samples/webSiteFabricatorLeft.vue'
...
export default defineComponent({
   ...
   methods: {
       async getCategory() {
           let fabricatorId = this.$route.params.fabricatorId
           const fabricatorResponse = await axios.get(`fabricator`)
           this.fabricator = fabricatorResponse.data.find(item => item.id === Number(fabricatorId))

           let notFilteredItems = []

           const itemsResponse = await axios.get('product')
           notFilteredItems = itemsResponse.data.filter(item => item.fabricator.id === Number(fabricatorId))
       ...
       async sendFeedBack() {
        ...
        filterItems() {
         ...
         this.$router.push({ path: `/fabricator/${this.fabricator.id}`, query: query_params, replace: true })
         },
         nullFilters() {
